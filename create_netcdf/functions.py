...
# ----------------------------------------------------------------
# Function to write 3d netCDF files with lake levels
...
# ----------------------------------------------------------------
# Function to write 3d netCDF files for years with lakepct

def write_netcdf_3d_lakepct(filename_rasters, filename_netcdf, attrs_variable, variable_name,attrs_global, years_start, years_end):
    ...
    for n,filename_raster in enumerate(filename_rasters):
        ...
        values_individual = np.array(raster.GetRasterBand(1).ReadAsArray())
        values_dict[n] = np.flipud(values_individual) 
    values = np.stack(values_dict.values()) * 100 # in % 
    ...
    years = np.arange(years_start,years_end+1)
    ...
    values_da = xr.DataArray(values, 
                            coords = {'time':years,'lon':lons,'lat':lats},
                            dims=('time','lat','lon'),
                            attrs = attrs_variable)
...
# ----------------------------------------------------------------
# Function to write 2d netCDF files for lakepct with 

def write_netcdf_2d_pctlake(filename_raster,filename_netcdf,attrs_variable,variable_name,attrs_global):
    ...
    raster = gdal.Open(filename_raster)
    values = np.array(raster.GetRasterBand(1).ReadAsArray())
    values = np.flipud(values) * 100 # to have it in [%]
